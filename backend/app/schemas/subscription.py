...
class PlanLimits(BaseModel):
    ...
    conversation_minutes_per_day: int = Field(
        description="1日あたりの会話練習時間（分）"
    )
    pronunciation_evaluations_per_day: int = Field(
        description="1日あたりの発音評価上限"
    )
...
class PlanInfo(BaseModel):
    ...
    features: list[PlanFeature] = Field(
        default_factory=list, description="含まれる機能一覧"
    )
...
class SubscriptionInfo(BaseModel):
    ...
    current_period_start: datetime | None = Field(
        default=None, description="現在の課金期間開始日"
    )
    current_period_end: datetime | None = Field(
        default=None, description="現在の課金期間終了日"
    )
    cancel_at_period_end: bool = Field(
        default=False, description="期間終了時にキャンセルされるか"
    )
...
class CheckoutSessionRequest(BaseModel):
    ...
    billing_cycle: str = Field(
        default="monthly", description="課金サイクル: monthly, yearly"
    )
...
class CheckoutSessionResponse(BaseModel):
    ...
    checkout_url: str = Field(
        description="StripeチェックアウトページへのリダイレクトURL"
    )
